...
<div class="col-md-12">
	...
	<div class="card-header ">
    		<h2 class="card-title" style="margin-right:65px;">Liste des documents
     ...
     <a  href="/docs" role="button"><button type="button" class="b" style="background:#00C8B0;margin-left:850px;">Nouveau Document</button></a>
									</div>
	<div class="card-body ">
	<div class="table-full-width">
		...
		<thead>
		<tr>
  <th width="19%" class="th">Patient</th>
	<th width="23%" class="th">Document</th>
	<th  width="20%" class="th">Date&Heure de Resultat </th>
  <th width="18%"></th>
</tr>
</thead>
<tbody>
    @foreach($doc as $d)
  <tr>
    <td class="td">{{$d->cin}}-{{$d->prenom}}{{$d->nom}}</td>
    <td class="td">{{$d->code}}-{{$d->description}} </td>
    <td class="td">{{$d->date_rslt}}</td>
    <td>

  <a  href="modifier{{$d->id}}" role="button">  <button type="button" class="b">Modifier</button></a>
  <a  data-target="#my-Modal-{{ $d->id }}"
...
<form method="get" action={{"destroy/".$d->id}} >
@csrf
...
<h2 class="modal-title" style="margin-top:15px;font-family:'Dosis',sans-serif;color:#014A6A;font-weight:bolder;font-size:15px; text-transform: uppercase;text-align:center;">
        Etes vous sur de vouloir supprimer ce document?</h2>
      <br/>
